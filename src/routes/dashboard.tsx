...
function Dashboard() {
  ...
  const [recent, setRecent] = useState<
    { id: string; status: string; other: Profile | null; direction: "in" | "out" }[]
  >([]);
  ...
  const refresh = useCallback(async () => {
    ...
    const [{ data: prof }, { data: requests }] = await Promise.all([
      ...
      supabase
        .from("swap_requests")
        .select(
          "id, status, requester_id, receiver_id, created_at, requester:profiles!swap_requests_requester_id_fkey(id,full_name,headline), receiver:profiles!swap_requests_receiver_id_fkey(id,full_name,headline)",
        )
          ...
          <div className="mt-6 glass rounded-2xl p-5 flex items-center justify-between flex-wrap gap-3 border border-[color:var(--teal)]/30">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Add your name and skills so others can match with you.
              </div>
            </div>
            <Link
              to="/profile"
              className="inline-flex items-center gap-1.5 text-sm font-semibold rounded-full bg-gradient-cta text-primary-foreground px-4 py-2 shadow-glow-cta"
            >
                ...
                <li key={r.id} className="py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-sm font-medium truncate">
                      {r.other?.full_name ?? "An engineer"}
                    </div>
                    <div className="text-xs text-muted-foreground truncate">
                      {r.other?.headline ?? "Software engineer"}
                    </div>
                  </div>
...
function StatusPill({ status, direction }: { status: string; direction: "in" | "out" }) {
  const map: Record<string, { label: string; cls: string }> = {
    pending: {
      label: direction === "in" ? "Pending · review" : "Pending · sent",
      cls: "bg-yellow-400/10 text-yellow-200 ring-yellow-400/30",
    },
  ...
  return (
    <span className={`text-[11px] font-medium px-2.5 py-1 rounded-full ring-1 ${m.cls}`}>
      {m.label}
    </span>
  );
}
